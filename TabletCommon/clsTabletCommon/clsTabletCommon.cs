...
namespace ITPAndroidApp
{
    public class clsTabletDB
    {
        ...
        {
            string[] colHeaderNames = { "AutoID", "TableName", "VersionNumber" };
            string[] colHeaderTypes = { "INTEGER PRIMARY KEY AUTOINCREMENT", "[nvarchar](250) NOT NULL", "[float] NULL" };
            string[] colHeaderBaseTypes = { "autoincrement", "string", "float" };
            ...
            public bool UpdateVersionNumber(string sTableName, double dNewVersionUmber)
            {
                try
                {
                    ...
                    {
                        sSQL = "Update " + sDocITPStaticTableName + " set VersionNumber = " + dNewVersionUmber + " where TableName = '" + sTableName + "'";
                    }
                    else
                    {
                        sSQL = "Insert into " + sDocITPStaticTableName + " (TableName, VersionNumber) Values ('" + sTableName + "'," + dNewVersionUmber + ")";
                    }
